...
describe('MenuComponent', () => {
  ...
  it('should listen to userEvents and score updates in ngOnInit', fakeAsync(() => {
    ...
    expect(component.user).withContext('Your component should listen to the `userEvents` observable on login').toBe(user);
    ...
    expect(component.user.money).withContext('Your component should listen to the `scoreUpdates` observable').toBe(300);

    // emulate an error
    fakeScoreUpdates.error('You should catch potential errors on score updates with a `.catch()`');
    tick();
    expect(component.user.money).withContext('Your component should catch error on score updates').toBe(300);
    ...
    expect(component.user.money).withContext('Your component should catch error on score updates').toBe(400);

    // emulate a logout
    fakeUserService.userEvents.next(null);
    tick();

    expect(component.user).withContext('Your component should listen to the `userEvents` observable on logout').toBe(null);
  }));
  ...
  it('should display the user if logged', () => {
    ...
    expect(info)
      ...
      .not.toBeNull();
    expect(info.textContent).withContext('You should display the user\'s name in a `span` element').toContain('cedric');
    expect(info.textContent).withContext('You should display the user\'s score in a `span` element').toContain('200');
  });
