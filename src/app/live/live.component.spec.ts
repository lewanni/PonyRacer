...
describe('LiveComponent', () => {
  ...
  it('should subscribe to the live observable if the race is PENDING', async(() => {
    ...
    expect(liveComponent.positionSubscription).withContext('positionSubscription should store the subscription').not.toBeNull();
  }));
  ...
  it('should subscribe to the live observable if the race is RUNNING', async(() => {
    ...
    expect(fakeRaceService.live).toHaveBeenCalledWith(1);
    expect(liveComponent.positionSubscription).withContext('positionSubscription should store the subscription').not.toBeNull();
    expect(liveComponent.poniesWithPosition.length).withContext('poniesWithPositions should store the positions').toBe(1);
  }));
  ...
  it('should switch the error flag if an error occurs', async(() => {
    ...
    expect(liveComponent.error).withContext('You should store that an error occurred in the `error` field').toBeTruthy();
  }));
  ...
  it('should tidy things up when the race is over', async(() => {
    ...
    expect(liveComponent.poniesWithPosition.length).withContext('poniesWithPositions should store the positions').toBe(3);
    expect(liveComponent.winners).withContext('The winners should be empty until the race is over').not.toBeUndefined();
    expect(liveComponent.winners).toEqual([]);
    expect(liveComponent.betWon).withContext('The bet status should be undefined until the race is over').toBeUndefined();

    positions.complete();
    expect(liveComponent.raceModel.status).withContext('The race status should change to FINISHED once the race is over').toBe('FINISHED');
    expect(liveComponent.winners).withContext('The winners should be not undefined once the race is over').not.toBeUndefined();
    expect(liveComponent.winners.length).withContext('The winners should contain all the ponies that won the race').toBe(2);
    expect(liveComponent.winners.map(pony => pony.id)).toEqual([1, 2], 'The winners should contain all the ponies that won the race');
    expect(liveComponent.betWon).withContext('The bet status should not be undefined until the race is over').not.toBeUndefined();
    expect(liveComponent.betWon).withContext('The bet status should true if the player won the bet').toBeTruthy();
  }));
  ...
  it('should display the pending race', () => {
    ...
    expect(title).withContext('The template should display an h2 element with the race name inside').not.toBeNull();
    expect(title.textContent).withContext('The template should display an h2 element with the race name inside').toContain('Lyon');
